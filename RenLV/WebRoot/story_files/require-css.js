define(function() {
    ...
    if(!__loadSrc__ && window.location.href.indexOf('__qJSDebug__=true') != -1){__loadSrc__=true; }
    ...
    var linkLoad = function(url, callback,moduleName) {
        ...
        if(window.__qRequire__ && window.__qRequire__.combineCSS && window.__qRequire__.combineCSS.length ){
           for(var i=0,len=window.__qRequire__.combineCSS.length;i<len;i++){
                if(window.__qRequire__.combineCSS[i] == moduleName){
                    return callback();
                }
            }
        }
